...
class sensorInputHandler {
    ...
    public _updateTrack(newCondition) {

        this._translateTrack.value = parseInt(newCondition);
    }
    public _updateField(field, newCondition) {
    ...
    public _updateProperty(property, value) {
    ...
    public _handleTranslation(translationName, translationValue) {
    ...
    public _handlePointerMove(event) {
    ...
    public _calcDistance(x1, y1, x2, y2) {
        return Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2));
    }
    public _handleScaling(prevDistance, currDestance) {
    ...
    public _calcAngleFromTriangle(A, B, C) {
        return (Math.pow(A, 2) + Math.pow(B, 2) - Math.pow(C, 2)) / (2 * A * B);
    }

    public _handlePointerRotation(point1, point2) {
        const circleCenter = {
    ...
    public _calcChanges(conditionName, eventCondition) {
        ...
        if (Math.abs(eventCondition - this._prevConditions[conditionName]) > 5) {
            const newConditionValue = (this._prevConditions[conditionName] < eventCondition) ? this._transformProperties[conditionName] + 5 : this._transformProperties[conditionName] - 5;
    ...
    public _handleTouchRotation(eventRotation) {
